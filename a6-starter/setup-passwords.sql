...
BEGIN
  ...
  SET salted_password = SHA2(CONCAT(salt, password), 256);
  INSERT INTO user_info(username, salt, password_hash) VALUES (new_username, salt, salted_password);
END !
DELIMITER ; 
...
BEGIN
  ...
  DECLARE input_password BINARY(64);
  ...
  SET pass_hash = SHA2(CONCAT((SELECT salt FROM user_info WHERE user_info.username = 
     username), password), 256);
  SET input_password = (SELECT password_hash FROM user_info WHERE user_info.username 
     = username);

  -- Checks if the provided password, when salted and hashed, matches the stored hash
  IF pass_hash = input_password THEN
    RETURN 1;
  ELSE 
    RETURN 0;
  END IF;
...
CALL sp_add_user('cloudly', 'hi');
CALL sp_add_user('lea', 'bye');
...
BEGIN
  ...
  DECLARE new_salted_password BINARY(64);

  SET new_salt = make_salt(8);

  SET new_salted_password = SHA2(CONCAT(new_salt, new_password),256);
  UPDATE user_info SET salt = new_salt, password_hash = new_salted_password 
